...
const DEFAULT_ERRORS: Record<ErrorType, Omit<ErrorConfig, "type">> = {
  ...
  [ErrorType.PROVIDER_ERROR]: {
    ...
    description:
      "Unable to connect to the Ethereum network provider. Please check your wallet connection.",
    action: {
      label: "Try Again",
      onClick: () => window.location.reload(),
    },
  ...
  [ErrorType.INSUFFICIENT_FUNDS]: {
    ...
    description:
      "You don't have enough ETH in your wallet to complete this transaction. You need ETH for:\n1. The donation amount\n2. Gas fees",
    action: {
  ...
  [ErrorType.METAMASK_PENDING]: {
    ...
    description:
      "Please check MetaMask for a pending connection request. Click the MetaMask icon in your browser extensions to view the request.",
    action: {
      label: "Try Again",
      onClick: () => window.location.reload(),
    },
  ...
  [ErrorType.GET_CAMPAIGNS_FAILED]: {
    ...
    description:
      "Unable to load campaigns from the blockchain. This could be due to network issues or contract problems.",
    action: {
      label: "Try Again",
      onClick: () => window.location.reload(),
    },
  ...
  [ErrorType.CREATE_CAMPAIGN_FAILED]: {
    ...
    description:
      "Unable to create the campaign. Please check your inputs and try again.",
    action: {
      label: "Try Again",
      onClick: () => window.location.reload(),
    },
  ...
  return (
    <div className="fixed top-4 right-4 z-50 max-w-md bg-dark-800 rounded-lg shadow-lg p-4 border border-dark-700 animate-slide-in">
      <div className="flex items-start gap-3">
        <div className="flex-shrink-0">{ERROR_ICONS[error.type]}</div>
...
export const useError = () => {
  ...
  const showError = useCallback(
    (type: ErrorType, customConfig?: Partial<Omit<ErrorConfig, "type">>) => {
